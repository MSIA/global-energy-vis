...
var tip = d3.tip()
            ...
            .offset([-10, 0])
            .html(function(d) {
			  return "<strong>Country: </strong><span class='details'>" + d.properties.name + "<br></span>" + "<strong>"+currentLabel+": </strong><span class='details'>" + numberFormat(d.currentKey) +"</span>";
            })
...
var svgslider = d3.select("#vis")
    .append("svg")
    .attr("width",1300)//4/5 * (width + margin.left + margin.right))
...
var slider = svgslider.append("g")
    .attr("class", "slider")
    .attr("transform", "translate(" + (margin.left + 100) + "," + 10 /*height/6*/ + ")");
...
function first_func(ndata , full){
	...
	svg.append("path")
	.datum(topojson.mesh(ndata.features, function(a, b) { return a.id !== b.id; }))
	.attr("class", "names")
	.attr("d", path);

    full_data.forEach(function(d) { d.Year = +d.Year;});
    ready(currentKey, currentYear);
}
...
function ready(currentKey, currentYear) {  
	  ...
	  if(currentKey != currentKeyPrev) {
        ...
        startYear = d3.min(full_cur , function(d) {return +d.Year});
	    endYear = d3.max(full_cur , function(d) {return +d.Year});
		
		currentKeyPrev = currentKey;
		
		minE = d3.min(full_cur, function(d) { return +d[currentKey]; });
		maxE = d3.max(full_cur, function(d) { return +d[currentKey]; });
		intColor2 = d3.scaleSequential(d3.interpolateReds)
...
slider.insert("g", ".track-overlay")
  ...
  .selectAll("text")
	...
	endYearPrev = endYear;
	  ...
	  full_data2.forEach(function(d) { usageById[d.Code] = +d[currentKey]; });
	
	  //geo_data.features.forEach(function(d) { d[currentKey] = usageById[d.id] });
	  geo_data.features.forEach(function(d) { d.currentKey = usageById[d.id] });
	 
	  full_data2.forEach(function(d) {lusageById[d.Code] = Math.log(+d[currentKey])});  
...
]).then( function(data){first_func(data[0], data[1])});
